...
const joinUser = (req, res) => {
    ...
    conn.query(sql, values, (err, results) => {
        if (err) {
            console.error(err);
            return res.status(StatusCodes.BAD_REQUEST).json({ error: '데이터베이스 오류입니다.' });
        }

        return res.status(StatusCodes.CREATED).json({ message: `${values[0]} 회원가입 성공` });
    });
};
...
const login = (req, res) => {
    ...
    conn.query(sql, email, (err, results) => {
        ...
        if (loginUser && loginUser.password === hashPassword) {
            ...
            return res.status(StatusCodes.OK).json({ message: `${loginUser.email} 님 로그인 성공하였습니다.` });
        } else {
